...
util.resolve = function(basepath, input, _default, rule) {
  ...
  if(_default) paths.push(path.resolve(basepath, _default));
  
  for (var i in paths){
    if (!paths[i]) continue;
    //bit of a messy hack with the try/catch and statSync, to emulate a synchronous fs.exists
    try{
